...
def mainFunc():
    global recfile, WavQueue, subprocesses, systemInterupt,RecordingStatus,RecordingDuration,RecordingDirectory
    ...
    try:
        ...
        d1.start()
        d1.join(1.0)
        while True and RecordingStatus:
            try:
...
def execSubprocess(sub):
    ...
    try:
        global WavQueue, systemInterupt,RecordingDuration,RecordingStatus
        if sub == 1:
            print 'systemInterupt', systemInterupt
            count =1;
            while systemInterupt == False and RecordingStatus:
                try:
                    ...
                    if WavQueue.empty():
                        ...
                        if os.path.isfile(wavefile):
                            ...
                            cmd1 = 'ffmpeg -i '+wavefile+' -vn -ar 48000 -ac 2 -ab 128k -f mp3 '+mp3file+' -y -loglevel error -nostdin'
                            rsl1 = subprocess.call(cmd1, shell=True)
                            os.remove(wavefile)
                            # return 1
                        # if count >=3:
